...
void Level::load(const char* levelName)
{
	...
	if(levelName != NULL)
    {
        Json::Value root;//location for the object
        Json::Reader reader;
        ...
        const Json::Value tileTypes = root["TileType"];	
        
        for(int i = 0; i < tileTypes.size(); i++)
        {
        	int tiles = tileTypes[i].get("type", NULL).asInt();//converts tiletypes as an int
        
            setTileTypeAtIndex((TileType)tiles,i);
